...
const validRegistrationOptions = (config?: RegistrationOptions) =>
	config?.phoneNumberCountryCode && config.phoneNumberNationalNumber && config.phoneNumberMobileCountryCode;
...
export const makeRegistrationSocket = (config: SocketConfig) => {
	...
	const register = async (code: string) => {
		...
		const result = await mobileRegister(
			{ ...sock.authState.creds, ...(sock.authState.creds.registration as RegistrationOptions), code },
			config.options,
		);

		const meId = jidEncode(result.login!, 's.whatsapp.net');
