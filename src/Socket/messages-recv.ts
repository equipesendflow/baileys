...
export const makeMessagesRecvSocket = (config: SocketConfig) => {
	...
	const sendMessageAck = async ({ tag, attrs, content }: BinaryNode) => {
		...
		await sendNode(stanza).catch(error => onUnexpectedError(error, 'sending ack'));
	};
	...
	const handleMessage = async (node: BinaryNode) => {
		...
		const decryptMessageData = decryptMessageNode(node, authState.creds.me!.id, signalRepository, logger);
	...
	const handleMessageResponse = async (node: BinaryNode) => {
		...
		// 	} else {
		// 		logger.warn(node, 'could not send message again, as it was not found')
		// 	}
		// }
