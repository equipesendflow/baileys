...
export const makeSocket = ({
	...
	function shouldIgnoreFrameLog(frame: BinaryNode) {
		if (frame.tag === 'iq' && frame.attrs.xmlns === 'w:p') return true;

		if (ignoreIds.includes(frame.attrs.id)) return true;
	...
	const onMessageRecieved = (data: Buffer) => {
		noise.decodeFrame(data, frame => {
			...
			if (!(frame instanceof Uint8Array)) {
				...
				if (!anyTriggered && logger.level === 'trace') {
					logger.trace({ unhandled: true, msgId, fromMe: false, frame }, 'communication recv');
				}
			}
		});
	};

	const end = (error?: Error | undefined, silent?: boolean) => {
		if (closed) {
			return;
		}
		...
		if (!silent) {
			logger.error(error, error ? 'connection errored' : 'connection closed');
		}
	...
	return (error: Error) => {
		handler(
			new Boom(`WebSocket Error (${error?.message})`, {
				statusCode: error ? getCodeFromWSError(error) : 408,
				data: error,
			}),
		);
	};
