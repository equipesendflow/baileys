 ...
 */
export function decryptPollVote(pollEncValue: proto.IPollEncValue, pollContext: PollContext) {
	const { encPayload, encIv } = pollEncValue;
	const { pollCreatorJid, pollMsgId, pollEncKey, voterJid } = pollContext;
...
const processMessage = async (message: proto.IWebMessageInfo, context: ProcessMessageContext) => {
	...
	if (protocolMsg) {
		...
		if (content.pollUpdateMessage.pollCreationMessageKey?.id === 'ignore this block') {
