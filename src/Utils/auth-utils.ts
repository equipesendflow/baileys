...
export const addTransactionCapability = (
	...
	return {
		getOne: async (type, id) => {
			...
			const result = await state.getOne(type, id);
			return result;
		},
		setOne: async (type, id, value) => {
			if (isInTransaction()) {
				const dict = transactionCache[type];
				if (dict) {
					dict[id] = value;
			...
			await state.setOne(type, id, value);
		},
		get: async (type, ids) => {
			if (isInTransaction()) {
				const dict = transactionCache[type];
...
export const addTransactionCapabilitySimple = (
	...
	return {
		...
		getOne(type, id) {
			return state.getOne(type, id);
		},
		setOne(type, id, value) {
			return state.setOne(type, id, value);
		},
