			...
			if (Array.isArray(stanza.content)) {
				for (const { tag, attrs, content } of stanza.content) {
					...
					try {
						...
						switch (e2eType) {
							case 'skmsg':
								msgBuffer = await repository.decryptGroupMessage(sender, author, content);
								break;
							case 'pkmsg':
							case 'msg':
								const user = isJidUser(sender) ? sender : author;
								msgBuffer = await repository.decryptMessage(user, e2eType, content);
								break;
							default:
						...
						logger.debug({ key: fullMessage.key, err }, 'failed to decrypt message');
