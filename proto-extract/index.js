...
(async () => {
		...
		walk.simple(value, {
			VariableDeclarator(node) {
				if (
					...
					wantedModules.indexOf(node.init.arguments[0].value) !== -1
				) {
					...
					members = members.map(({ key: { name }, value: { elements } }) => {
						...
						unwrapBinaryOr(elements[1]).forEach(m => {
							if (m.type === 'MemberExpression' && m.object.type === 'MemberExpression') {
								if (m.object.property.name === 'TYPES') type = m.property.name.toLowerCase();
	...
	let decodedProto = ['syntax = "proto2";', 'package proto;', ''];
	...
	for (const mod of modules) {
		...
		const stringifyMessageSpecMember = (info, path, completeFlags = true) => {
			if (info.type === '__oneof__') {
				return [].concat(
					[`oneof ${info.name} {`],
					addPrefix(
						[].concat(...info.members.map(m => stringifyMessageSpecMember(m, path, false))),
						spaceIndent,
					),
					['}'],
				);
				...
				ret.push(
					`${info.flags.join(' ') + (info.flags.length === 0 ? '' : ' ')}${unnestedType} ${
						info.name
					} = ${info.id}${info.packed || ''};`,
				);
				return ret;
		...
		const stringifyMessageSpec = ident => {
			...
			result.push(
				...
				...addPrefix(
					[].concat(...ident.members.map(m => stringifyMessageSpecMember(m, ident.name))),
					spaceIndent,
				),
				'}',
			);
		...
		const identifiers = Object.keys(modInfo.identifiers)
			.sort()
			.map(d => modInfo.identifiers[d]);

		decodedProto = decodedProto.concat(...Object.values(identifiers).map(stringifyEntity));
	}
